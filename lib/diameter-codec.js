...
exports.constructRequest = function(applicationName, commandName, sessionId) {
    ...
    if(_.parseInt(command.code) !== 257){
...
var encodeAvp = function(avp, appId) {
    ...
    if (avpTag.type === 'Grouped') {
	  ...
	  if(avp[0] === "Framed-IP-Address" || avp[0] === 8){
            var ip = ipaddr.parse(value);
            var typeBuffer = new Buffer(2);
            typeBuffer.writeUInt8(0, 0);
            if (ip.kind() === 'ipv4') {
                typeBuffer.writeUInt8(1, 1);
                avpDataBuffer =  Buffer.concat([new Buffer(ip.toString().split('.'))]);
            } else {
                typeBuffer.writeUInt8(2, 1);
                avpDataBuffer = Buffer.concat([typeBuffer, new Buffer(ip.toByteArray())]);
            }
        }    
    }
